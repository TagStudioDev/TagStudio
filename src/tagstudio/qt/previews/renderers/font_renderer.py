...
class FontRenderer(BaseRenderer):
    ...
    def render(context: RendererContext) -> Image.Image | None:
        ...
        if context.is_grid_thumb:
            return _font_short_thumb(context)
        else:
            return _font_long_thumb(context)
...
def _font_short_thumb(context: RendererContext) -> Image.Image | None:
    ...
    """
    try:
        bg: Image.Image = Image.new("RGB", (context.size, context.size), color="#000000")
        raw: Image.Image = Image.new("RGB", (context.size * 3, context.size * 3), color="#000000")
        draw = ImageDraw.Draw(raw)
        font: FreeTypeFont = ImageFont.truetype(context.path, size=context.size)
...
def _font_long_thumb(context: RendererContext) -> Image.Image | None:
    ...
    try:
        scaled_sizes: list[int] = [math.floor(x * (context.size / 256)) for x in FONT_SAMPLE_SIZES]
        bg: Image.Image = Image.new("RGBA", (context.size, context.size), color="#00000000")
        draw = ImageDraw.Draw(bg)
        lines_of_padding: int = 2
        y_offset: float = 0.0

        for font_size in scaled_sizes:
            font: FreeTypeFont = ImageFont.truetype(context.path, size=font_size)
