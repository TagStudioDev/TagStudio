...
from PySide6.QtCore import QCoreApplication, QMetaObject, QRect, QSize, Qt, QSettings
...
class Ui_Settings(QDialog):
    ...
    def setupUi(self, Settings):
        ...
        self.spinBox.setMaximum(os.cpu_count())
    ...
    def getSettings(self):
        ...
        settings["render_thumbnails"] = self.render_thumbnails.isChecked()
        settings["limit_threads"] = self.checkBox.isChecked()
        settings["max_threads"] = self.spinBox.value()
        return settings

    def setSettings(self, settings: QSettings):
        if settings.contains("autosave"):
            self.autosave_toggle.setChecked(settings.value("autosave", True, bool))

        if settings.contains("autosave_interval"):
            self.autosave_interval.setValue(settings.value("autosave_interval", 5, int))

        if settings.contains("render_thumbnails"):
            self.render_thumbnails.setChecked(
                settings.value("render_thumbnails", False, bool)
            )

        if settings.contains("limit_threads"):
            self.checkBox.setChecked(settings.value("limit_threads", False, bool))

        if settings.contains("max_threads"):
            self.spinBox.setValue(settings.value("max_threads", 4, int))
