...
class TagSearchPanel(PanelWidget):
    ...
    def update_tags(self, query: str):
        ...
        if query:
            first_id_set = False
            for tag_id in self.lib.search_tags(query, include_cluster=True)[
                : self.tag_limit - 1
            ]:
                if not first_id_set:
                ...
                ab.clicked.connect(
                    lambda checked=False, x=tag_id: self.tag_chosen.emit(x)
                )
    ...
    def update_tags(self, query: str):
        ...
        if query:
            ...
            for tag_id in self.lib.search_tags(query, include_cluster=True)[
                ...
                tw.on_edit.connect(
                    lambda checked=False, t=self.lib.get_tag(tag_id): (
                        self.edit_tag(t.id)
                    )
                )
                l.addWidget(tw)
...
class FoldersToTagsModal(QWidget):
    ...
    class TreeItemTest(QWidget):
        def __init__(self, data: dict, parentTag: Tag):
            ...
            self.tag_widget = FoldersToTagsModal.ModifiedTagWidget(
                data["tag"], parentTag
            )
